		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			_push(ssrRenderComponent(StoreLayout_default, null, {
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) _push(`<section class="payment"${_scopeId}><p class="eyebrow"${_scopeId}>SANDBOX · FAKE PROVIDER</p><h1${_scopeId}>Тестова оплата</h1><p${_scopeId}>Замовлення ${ssrInterpolate(__props.payment.order.number)}</p><div class="pay-card"${_scopeId}><span${_scopeId}>До сплати</span><b${_scopeId}>${ssrInterpolate((__props.payment.amount / 100).toLocaleString("uk-UA"))} ₴</b><button class="button"${_scopeId}>Імітувати успішну оплату</button></div><small${_scopeId}>Жодні карткові дані не передаються.</small></section>`);
						...
						createVNode("p", null, "Замовлення " + toDisplayString(__props.payment.order.number), 1),
						createVNode("div", { class: "pay-card" }, [
							...
							}, "Імітувати успішну оплату", 8, ["onClick"])
						]),
