...
Кошик використовує off-canvas drawer: відкривається з header або автоматично
після додавання, не забирає покупця з картки товару, показує варіант/SKU, керує
кількістю в межах доступного залишку, дозволяє видалення та веде до guest
checkout. Окрема сторінка кошика лишається доступною як fallback.
